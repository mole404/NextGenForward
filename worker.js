...
const DEFAULT_SPAM_RULES = {
  version: 1,
  max_links: 1,                 // 文本中链接数 >= max_links 判定为 spam；0 表示不启用
  keywords: [
...
function sanitizeSpamRules(rules) {
  ...
  const safe = {
    ...
    ai: {
      ...
      threshold: (() => {
        const t = Number(r.ai && typeof r.ai === "object" ? r.ai.threshold : DEFAULT_SPAM_RULES.ai.threshold);
        return (Number.isFinite(t) && t >= 0 && t <= 1) ? t : DEFAULT_SPAM_RULES.ai.threshold;
      })()
    }
  };
  return safe;
...
function promptToSpamRules(promptText, baseRules) {
  ...
  let rules = sanitizeSpamRules(useDefaults ? (baseRules || DEFAULT_SPAM_RULES) : {
    version: 1,
    max_links: 0,
    keywords: [],
    regexes: [],
    allow_keywords: [],
    allow_regexes: [],
    ai: { enabled: false, model: "@cf/meta/llama-3.1-8b-instruct-fast", threshold: 0.85 }
  });
...
async function aiSpamVerdict(env, text, rules) {
  ...
  if (!rules || !rules.ai || !rules.ai.enabled) return null;
  ...
  const schema = {
    ...
    required: ["is_spam", "confidence", "category", "signals"]
  };

  try {
    const out = await env.AI.run(rules.ai.model, {
      messages: [
        { role: "system", content: "你是垃圾消息分类器。判断文本是否为垃圾消息（广告/引流/诈骗/推广）。只输出 JSON。" },
        { role: "user", content: JSON.stringify({ text: String(text || "").slice(0, 2000) }) }
      ],
      response_format: {
        type: "json_schema",
        json_schema: schema
    ...
    if (!r || typeof r.is_spam !== "boolean") return null;
    const conf = Number(r.confidence);
    const confidence = (Number.isFinite(conf) && conf >= 0 && conf <= 1) ? conf : 0;
    return {
      is_spam: !!r.is_spam,
      score: confidence,
      reason: `ai:${r.category || "unknown"}`,
...
async function classifySpamOptional(env, msg) {
  ...
  if (ai) {
    const isSpam = ai.is_spam && ai.score >= rules.ai.threshold;
